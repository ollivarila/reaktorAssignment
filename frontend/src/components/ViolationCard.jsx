...
const IconAndValue = ({ icon, value }) => <div style={{display: 'flex', gap: '2px'}}>{icon} {value}</div>

const ViolationCard = ({ name, email, phone, closestDistance, currentDistance, updatedAt }) => {
  const now = new Date(Date.now())
  const updatedDate = new Date(updatedAt)
  const lastSeenSeconds = ((now.getTime() - updatedDate.getTime()) / 1000).toFixed(0)
  ...
  return (
    <Grid item xs={4}>
      ...
      <Card sx={{ display: 'flex', flexDirection: 'column', padding: '10px'}}>
        ...
        <Box>
          <Typography sx={{ justifySelf: 'start'}}>~ {lastSeen} {unit} ago</Typography>
        </Box>
      </Card>
    </Grid>
  )
